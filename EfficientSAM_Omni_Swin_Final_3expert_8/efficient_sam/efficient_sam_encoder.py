...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        self.cls_emb = nn.Parameter(torch.randn(1, task_num, patch_embed_dim))
        self.sls_emb = nn.Parameter(torch.randn(1, scale_num, patch_embed_dim))
        self.task_num = task_num

        self.mha_token = SelfAttentionModule(feature_dim=patch_embed_dim)

    def forward(self, x: torch.Tensor, task_id, scale_id) -> torch.Tensor:

        ori_img = x.detach().clone()

        self.cls_emb.requires_grad = True
        self.sls_emb.requires_grad = True

        features = torch.cat([self.cls_emb, self.sls_emb],1)
        output = self.mha_token(features)
        new_cls_emb = output[:,:self.task_num,:]
        new_sls_emb = output[:,self.task_num:,:]
