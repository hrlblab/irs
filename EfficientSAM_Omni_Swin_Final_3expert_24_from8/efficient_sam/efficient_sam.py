...
class EfficientSam(nn.Module):
    ...
    def __init__(
        ...
        self.fusionConv_giga = nn.Sequential(
            nn.GroupNorm(16, 1536),
            nn.ReLU(inplace=in_place),
            # conv3x3x3(256, 256, kernel_size=(1, 1, 1), padding=(0, 0, 0), weight_std=self.weight_std)
            nn.Conv2d(1536, 256, kernel_size=(1, 1), stride=1, padding=(0, 0), dilation=1, bias=False)
        )

        self.fusionConv_uni = nn.Sequential(
            nn.GroupNorm(16, 1024),
            nn.ReLU(inplace=in_place),
            # conv3x3x3(256, 256, kernel_size=(1, 1, 1), padding=(0, 0, 0), weight_std=self.weight_std)
            nn.Conv2d(1024, 256, kernel_size=(1, 1), stride=1, padding=(0, 0), dilation=1, bias=False)
        )
        ...
        self.cls_token_encoder_MLP_PrpSeg = nn.Conv2d(384,32 + 32 + 64 + 128 + 256, kernel_size=1, stride=1, padding=0)
        self.sls_token_encoder_MLP_PrpSeg = nn.Conv2d(384,32 + 32 + 64 + 128 + 256, kernel_size=1, stride=1, padding=0)

        self.cls_token_encoder_MLP_Swin = nn.Conv2d(384, 24 + 48 + 96 + 192 + 384, kernel_size=1, stride=1, padding=0)
        self.sls_token_encoder_MLP_Swin = nn.Conv2d(384, 24 + 48 + 96 + 192 + 384, kernel_size=1, stride=1, padding=0)
        ...
        # self.giga = timm.create_model("hf_hub:prov-gigapath/prov-gigapath", pretrained=True)
        # self.giga.eval()
        # self.giga.requires_grad_(False)
        #
        # self.uni = timm.create_model("vit_large_patch16_224", img_size=224, patch_size=16, init_values=1e-5,num_classes=0, dynamic_img_size=True)
        # self.uni.load_state_dict(torch.load(os.path.join('/local_storage/dengr/Continual_Learning', "pytorch_model.bin"), map_location="cpu"), strict=True)
        # self.uni.eval()
        # self.uni.requires_grad_(False)

    def extract_patches(self, batch_image, patch_size=224, stride=224):
        # Unfold to get sliding patches
        # Shape after unfold will be: (batch_size, channels, num_patches_height, num_patches_width, patch_size, patch_size)
        patches = batch_image.unfold(2, patch_size, stride).unfold(3, patch_size, stride)

        # Reshape to (batch_size * num_patches, channels, patch_size, patch_size)
        batch_size, channels, num_patches_h, num_patches_w, _, _ = patches.shape
        patches = patches.contiguous().view(batch_size * num_patches_h * num_patches_w, channels, patch_size,
                                            patch_size)
    ...
    def predict_masks(
        ...
        now_cls_token_PrPSeg = self.cls_token_encoder_MLP_PrpSeg(now_cls_token)
        now_sls_token_PrPSeg = self.sls_token_encoder_MLP_PrpSeg(now_sls_token)
        ...
        now_cls_token_Swin = self.cls_token_encoder_MLP_Swin(now_cls_token)
        now_sls_token_Swin = self.sls_token_encoder_MLP_Swin(now_sls_token)

        split_sizes = [24,48,96,192,384]
        cls_token_split_swin = torch.split(now_cls_token_Swin, split_sizes, dim=-3)
        sls_token_split_swin = torch.split(now_sls_token_Swin, split_sizes, dim=-3)

        enc1 = self.swin_backbone.encoder2(hidden_states_out[0] + cls_token_split_swin[0] + sls_token_split_swin[0])
        enc2 = self.swin_backbone.encoder3(hidden_states_out[1] + cls_token_split_swin[1] + sls_token_split_swin[1])
        enc3 = self.swin_backbone.encoder4(hidden_states_out[2] + cls_token_split_swin[2] + sls_token_split_swin[2])
        dec4 = self.swin_backbone.encoder10(hidden_states_out[4] + cls_token_split_swin[4] + sls_token_split_swin[4])
        ...
        # 'Gigapixel'
        # x_giga = self.extract_patches(ori_img)
        # with torch.no_grad():
        #     x_giga = self.giga(x_giga)
        #     x_giga = x_giga.view(ori_img.shape[0], 4, -1)
        #     x_giga = x_giga.view(ori_img.shape[0], 2, 2, -1).detach().clone()
        #
        # x_giga_fusion = self.fusionConv_giga(x_giga.permute([0, 3, 1, 2]))
        # x_giga_feat = self.GAP(x_giga_fusion)
        #
        # 'Uni'
        # x_uni = self.extract_patches(ori_img)
        # with torch.no_grad():
        #     x_uni = self.uni(x_uni)
        #     x_uni = x_uni.view(ori_img.shape[0], 4, -1)
        #     x_uni = x_uni.view(ori_img.shape[0], 2, 2, -1).detach().clone()
        #
        # x_uni_fusion = self.fusionConv_uni(x_uni.permute([0, 3, 1, 2]))
        # x_uni_feat = self.GAP(x_uni_fusion)

        'MoE attention'
        #output = self.mha_expert(torch.cat([x_omni_feat, x_efficientsam_feat, x_swin_feat, x_giga_feat, x_uni_feat],2).squeeze(-1).permute([0,2,1]))
        ...
        'swin_decoder'
        dec3 = self.swin_backbone.decoder5(dec4, hidden_states_out[3] + cls_token_split_swin[3] + sls_token_split_swin[3])
        ...
        return logits, moe_feat, now_cls_token, head_inputs
