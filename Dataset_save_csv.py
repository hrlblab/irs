...
class MOTSDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(64, 192, 192), mean=(128, 128, 128), scale=True,
        ...
        for ki in range(len(tasks)):
            ...
            #     continue
            layer_id = os.path.basename(tasks[ki]).split('_')[0]
            tasks_id = os.path.basename(tasks[ki]).split('_')[1]
            scale_id = os.path.basename(tasks[ki]).split('_')[2]
            ...
            for si in range(len(stain_folders)):
                ...
                for ri in range(len(images)):
                    ...
                    if 'lbl' in images[ri]:
                        ...
                        try:
                            mask_root = glob.glob(os.path.join(stain_folders[si],os.path.basename(image_root).replace('_img.png','_mask*').replace(ext,'_mask*')))[0]
                        except:
                            mask_root = glob.glob(os.path.join(stain_folders[si],
                                                               os.path.basename(image_root).replace('_img.png',
                                                                                                    '_lbl*').replace(
                                                                   ext, '_lbl*')))[0]
        ...
        self.files = []
        self.df = pd.DataFrame(columns = ['image_path', 'label_path', 'name', 'layer_id', 'task_id', 'scale_id'])
        ...
        for i in range(len(image_path_list)):
            ...
            self.df.loc[i] = [image_path, label_path, name, layer_id, task_id, scale_id]
...
if __name__ == '__main__':
    ...
    trainset_dir = '/Data4/IRS_github/data_step1/train/'
    train_list = '/Data4/IRS_github/data_step1/train/'
    itrs_each_epoch = 250
    batch_size = 1
    input_size = (256,256)
    random_scale = False
