...
def get_arguments():
    ...
    parser.add_argument("--trainset_dir", type=str, default='/Data4/IRS_github/data_step2/train/data_list_step2.csv')

    # parser.add_argument("--valset_dir", type=str, default='/Data2/KI_data_validationset_patch/data_list.csv')
    parser.add_argument("--valset_dir", type=str, default='/Data4/IRS_github/data_step2/val/data_list_step2.csv')
    ...
    parser.add_argument("--reload_path", type=str, default='snapshots_2D/MoE_3_step1/MOTS_DynConv_MoE_3_step1_e100.pth')
...
def supervise_learning_continual_HATs(images, labels, batch_size, scales, model, now_task, weight, HATs_matrix, loss_seg_DICE, loss_seg_CE, model_8class, loss_KL, loss_MSE):

    ori_class = [0, 1, 2, 3, 4, 6, 9, 10]
    pseudo_class = random.sample(ori_class, 3)
    HAts_relationship = HATs_matrix[pseudo_class, now_task]

    pseudo_logit = torch.zeros((len(pseudo_class), labels.shape[0], 2, labels.shape[1], labels.shape[2])).cuda()
    x_latent_list = torch.zeros((len(pseudo_class), labels.shape[0], 256, 1, 1)).cuda()
    class_token_list = torch.zeros((len(pseudo_class), labels.shape[0], 384, 1, 1)).cuda()
    head_input_list = torch.zeros((len(pseudo_class), 1, labels.shape[0] * 8, 512, 512)).cuda()

    old_class_logit_now = torch.zeros((len(pseudo_class), labels.shape[0], 2, labels.shape[1], labels.shape[2])).cuda()
    x_latent_list_now = torch.zeros((len(pseudo_class), labels.shape[0], 256, 1, 1)).cuda()
    class_token_now = torch.zeros((len(pseudo_class), labels.shape[0], 384, 1, 1)).cuda()
    head_input_list_now = torch.zeros((len(pseudo_class), 1, labels.shape[0] * 8, 512, 512)).cuda()

    semi_ratio = 1.
    now_area_ratio = 1.

    for ni in range(len(pseudo_class)):
        now_pseudo_label = pseudo_class[ni]
        now_preds, x_latent, now_class_token, now_head_input = model(images, torch.ones(batch_size).cuda() * now_pseudo_label, scales)
        old_class_logit_now[ni] = now_preds
        x_latent_list_now[ni] = x_latent
        class_token_now[ni] = now_class_token
        head_input_list_now[ni] = now_head_input

        with torch.no_grad():
            now_preds, x_latent, now_class_token, now_head_input = model_8class(images, torch.ones(batch_size).cuda() * now_pseudo_label, scales)
            pseudo_logit[ni] = now_preds.detach().clone()
            x_latent_list[ni] = x_latent.detach().clone()
            class_token_list[ni] = now_class_token.detach().clone()
            head_input_list[ni] = now_head_input.detach().clone()

    'HATs'
    HATs_term_seg_Dice = 0
    for ii in range(len(pseudo_class)):
        now_relative = HAts_relationship[ii]
        semi_preds = old_class_logit_now[ii]
        ...
        if now_relative == 1:
            ...
            HATs_term_seg_Dice -= semi_ratio * semi_seg_Dice * now_area_ratio
            ...
            semi_labels = one_hot_3D(semi_labels.long())
            semi_seg_Dice, semi_seg_BCE, semi_all = get_loss(images, semi_preds, semi_labels, weight, loss_seg_DICE,
                                                             loss_seg_CE)
            HATs_term_seg_Dice += semi_ratio * semi_seg_Dice * now_area_ratio
            #term_all += semi_ratio * semi_seg_Dice * now_area_ratio
    ...
    preds, _, _, _ = model(images, torch.ones(batch_size).cuda() * now_task, scales)

    new_label = labels
    labels = one_hot_3D(new_label.long())

    term_seg_Dice, term_seg_BCE, term_all = get_loss(images, preds, labels, weight, loss_seg_DICE, loss_seg_CE)

    term_KL1, term_MSE1, term_all_semi1 = semi_learning(pseudo_logit, old_class_logit_now, loss_KL, loss_MSE)
    term_KL2, term_MSE2, term_all_semi2 = semi_learning(x_latent_list, x_latent_list_now, loss_KL, loss_MSE)
    term_KL3, term_MSE3, term_all_semi3 = semi_learning(class_token_list, class_token_now, loss_KL, loss_MSE)
    term_KL4, term_MSE4, term_all_semi4 = semi_learning(head_input_list, head_input_list_now, loss_KL, loss_MSE)

    return term_seg_Dice, term_seg_BCE, term_all + 0.25 * term_all_semi1 + 0.25 * term_all_semi2 + 0.25 * term_all_semi3 + 0.25 * term_all_semi4 + HATs_term_seg_Dice
...
def semi_learning(feature_map1, feature_map2, loss_KL, loss_MSE):
    feature_map1 = feature_map1.float()
    feature_map2 = feature_map2.float()

    feature_map1 = torch.clamp(feature_map1, min=-1e10, max=1e10)
    feature_map2 = torch.clamp(feature_map2, min=-1e10, max=1e10)

    softmax = nn.Softmax(dim=2)
    epsilon = 1e-10
    term_KL = loss_KL(softmax(feature_map1) + epsilon, softmax(feature_map2) + epsilon)

    term_MSE = loss_MSE(feature_map1 + epsilon, feature_map2 + epsilon)
...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        new_reload_path = args.reload_path
        model.load_state_dict(torch.load(new_reload_path, map_location=torch.device('cpu')), strict=False)
        print('load successfully')
        model.image_encoder.requires_grad_(False)

        model_8class = build_efficient_sam_vits(task_num=task_num, scale_num=4)
        model_8class.requires_grad_(False)
        model_8class.load_state_dict(torch.load(args.reload_path, map_location=torch.device('cpu')), strict=False)
        print('load successfully')
        model_8class.cuda()
        model_8class.eval()
        ...
        matrix_csv = pd.read_csv('/Data4/IRS_github/HATs_matrix.csv')
        HATs_matrix = matrix_csv.values[1:, 1:]
        ...
        for epoch in range(0,args.num_epochs):
            ...
            for iter, batch in enumerate(trainloader):
                ...
                for now_task in range(task_num):  # Loop through tasks 0 to 22
                    ...
                    if task_pool_image.num_imgs >= batch_size:
                        ...
                        term_seg_Dice, term_seg_BCE, Sup_term_all = supervise_learning_continual_HATs(
                            images, labels, batch_size, scales, model, now_task, weight, HATs_matrix, loss_seg_DICE,
                            loss_seg_CE, model_8class, loss_KL, loss_MSE
                        )
            ...
            for task_id in range(task_num):  # Loop from task 8 to task 22
                ...
                if task_pool_image.num_imgs > 0:
                    ...
                    'supervise_learning'
                    term_seg_Dice, term_seg_BCE, Sup_term_all = supervise_learning_continual_HATs(images, labels, batch_size,
                                                                                             scales,
                                                                                             model, now_task, weight, HATs_matrix,
                                                                                             loss_seg_DICE, loss_seg_CE,
                                                                                             model_8class, loss_KL,
                                                                                             loss_MSE)
            ...
            if (epoch >= 0) and (args.local_rank == 0) and (((epoch % 10 == 0) and (epoch >= 800)) or (epoch % 1 == 0)):
                ...
                with torch.no_grad():
                        ...
                        for now_task in range(task_num):  # Loop through tasks 0 to 22
                            ...
                            if task_pool_image.num_imgs >= batch_size:
                                if now_task in size_1024:
                                    ...
                                    preds[:, :, :512, :512],_,_,_ = model(images[:, :, :512, :512],torch.ones(batch_size).cuda() * now_task, scales)
                                    preds[:, :, :512, 512:],_,_,_ = model(images[:, :, :512, 512:],torch.ones(batch_size).cuda() * now_task, scales)
                                    preds[:, :, 512:, 512:],_,_,_ = model(images[:, :, 512:, 512:],torch.ones(batch_size).cuda() * now_task, scales)
                                    preds[:, :, 512:, :512],_,_,_ = model(images[:, :, 512:, :512],torch.ones(batch_size).cuda() * now_task, scales)
                                    ...
                                    for bi in range(len(scales)):
                                        scales[bi] = task_scale.pop(0)
                                        layers[bi] = task_layer.pop(0)
                                    preds,_,_,_ = model(images[:, :, 256:768, 256:768], torch.ones(batch_size).cuda() * now_task, scales)
                    ...
                    class_name = ['0medulla','1cortex','2dt','3pt','4cap','5tuft','6art','7ptc','8mv','9pod','10mes','11gloendo','12glopecs','13smooth','14vesendo',
                                  '15Adhesion','16CapsularDrop','17GlobalSclerosis','18Haylinosis','19MesangialExpansion','20MesangialLysis','21Microaneurysm','22NodularSclerosis','23SegmentalSclerosis']
            ...
            if (epoch >= args.num_epochs - 1) and (args.local_rank == 1):
                print('save model ...')
                if args.FP16:
                    checkpoint = {
                        'model': model.state_dict(),
